#!/usr/bin/env python3
"""
Upload 1/100 of BAC RAG content to Upstash Vector to avoid rate limiting.
Only processes one subject at a time, uploads 1% of chunks.
"""
...
SLEEP = 2.0  # Increased sleep time
...
def upsert_to_upstash(vectors, retries=3):
...
def process_rag_file(filepath, subject, fraction=0.01):
    ...
    # Only process 1% of chunks to avoid rate limiting
    ...
    print(f"  Processing: {sample_size} chunks (1/100)")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 upload-1-100.py <subject>")
    ...
    # Upsert to Upstash in very small batches
    print(f"\nUploading to Upstash...")
    upstash_batch_size = 5  # Very small batches
    for i in range(0, len(vectors_to_upsert), upstash_batch_size):
        ...
        time.sleep(2.0)  # Longer sleep between batches
