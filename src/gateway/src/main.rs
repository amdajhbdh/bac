...
impl OCRClient {
    ...
    pub async fn process_image(&self, image_data: Vec<u8>, file_name: &str) -> Result<OCRResponse, String> {
        ...
        if VERBOSE.load(Ordering::Relaxed) {
            eprintln!("OCR: processing {} ({} bytes)", file_name, image_data.len());
        }
        ...
        if VERBOSE.load(Ordering::Relaxed) {
            eprintln!("OCR: done {}ms confidence:{:.1}%", 
                start.elapsed().as_millis(), 
                ocr_response.data.as_ref().map(|d| d.confidence * 100.0).unwrap_or(0.0));
        }
    ...
    pub async fn process_pdf(&self, pdf_data: Vec<u8>) -> Result<OCRResponse, String> {
        let start = Instant::now();
        if VERBOSE.load(Ordering::Relaxed) {
            eprintln!("OCR: PDF {} bytes", pdf_data.len());
        }
        ...
        if VERBOSE.load(Ordering::Relaxed) {
            eprintln!("OCR: PDF done {}ms", start.elapsed().as_millis());
        }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!("BAC Gateway v1.0.0 starting...");
